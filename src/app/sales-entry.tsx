...
type LimitThreshold = Omit<NumberLimit, "number">;
...
export default function SalesEntry({
  ...
  dealerNumberTotals: Record<string, number>;
  allNumberLimit?: LimitThreshold;
  globalNumberLimits: NumberLimit[];
  commissionNumberLimits: NumberLimit[];
  summaryTotal: number;
  ...
  payoutRate: number;
  winningDetails: { receiptNumber: number; amount: number }[];
  winningPeriodId?: string;
  ...
  const limitStatus = (number: string) => {
    ...
    const limit = commissionLimit || globalLimit || allNumberLimit;
    if (!limit) return "";
    const amount = commissionLimit ? numberTotals[number] || 0 : dealerNumberTotals[number] || 0;
    if (amount >= Number(limit.max_amount)) return "over";
    if (amount >= (Number(limit.max_amount) * Number(limit.warning_percent)) / 100) return "warning";
    return "";
  };
  const addNumberToInput = (number: string) => {
    const nextValue = rawInput
      ? `${rawInput.replace(/\s*$/, "")}\n${number} `
      : `${number} `;
  ...
  const changeScope = (key: "commission" | "period", value: string) => {
  ...
  useEffect(() => {
    ...
    const selects = Array.from(
      form.querySelectorAll<HTMLSelectElement>(
        'select[name="commission_id"], select[name="draw_period_id"]',
      ),
    );
    const handlers = selects.map((select) => {
      const key = select.name === "commission_id" ? "commission" : "period";
      const selected = searchParams.get(key);
      if (selected) select.value = selected;
      const handler = () => changeScope(key, select.value);
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,.8fr)]">
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-blue-700">အရောင်းစာရင်း</p>
        ...
        <form ref={formRef} action={formAction} onSubmit={submitOptimistically} className="space-y-4">
          ...
          <label className="block text-sm text-slate-600">
            ...
            <textarea
              ...
              rows={5}
              placeholder={"ဥပမာ\n45650\n345897345098150\nA100\n123R100\n123.654.980.342R100"}
              className="mt-1.5 w-full resize-y rounded-lg border border-slate-300 px-3 py-3 text-base leading-7 placeholder:text-slate-400"
            />
          </label>
          <p className="text-xs text-slate-500">
            Enter = စာရင်းသိမ်းမည် · Shift + Enter = line အသစ် · `45650` နှင့် 3လုံးအုပ်စုများကို `.` ပါ/မပါ ရိုက်နိုင်သည်
          </p>
          {state.error && (
        ...
        <div className="mt-6 border-t border-slate-200 pt-5">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="font-semibold">အရောင်းစာရင်း</h3>
            <span className="text-sm text-slate-500">
              နောက်ဆုံးထည့်ထားသည်များ
            </span>
          </div>
            ...
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs text-slate-500">
                <tr>
                  ...
                  <th className="px-3 py-2">စာရင်း</th>
                  <th className="px-3 py-2 text-right">စုစုပေါင်း</th>
                  <th className="px-3 py-2">အချိန်</th>
                  <th className="px-3 py-2 text-center">အသေးစိတ်</th>
                  <th className="px-3 py-2 text-center">လုပ်ဆောင်ချက်</th>
                </tr>
              </thead>
              <tbody>
                {sales.map((sale) => {
                    ...
                    {sale.raw_input}{sale.status === "deleted" && <span className="ml-2 rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-semibold text-rose-700">ဖျက်ထားသည်</span>}
                        ...
                        <td className="px-3 py-3 text-center">
                          <button
                            ...
                            className="rounded-md border border-slate-200 px-2 py-1 text-xs text-blue-700 hover:bg-blue-50"
                          >
                            {expanded ? "ဖျောက်" : "ကြည့်"}
                          </button>
                        </td>
                        <td className="px-3 py-3 text-center">
      ...
      <aside className="space-y-5">
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold">ဂဏန်းအလိုက် ရောင်းအား</h2>
            ...
            <div className="flex items-center gap-1">
              <button
                type="button"
                onClick={() => setPrefix((value) => (value + 9) % 10)}
                aria-label="ယခင်ရှေ့ဂဏန်း"
                className="h-7 w-7 rounded-md border border-slate-300 text-sm"
              >
                ‹
              </button>
              <span className="min-w-7 text-center text-lg font-bold text-blue-700">
                {prefix}
              </span>
              <button
                type="button"
                onClick={() => setPrefix((value) => (value + 1) % 10)}
                aria-label="နောက်ရှေ့ဂဏန်း"
                className="h-7 w-7 rounded-md border border-slate-300 text-sm"
              >
                ›
              </button>
          ...
          <div className="number-grid mt-3 grid grid-cols-4 overflow-hidden rounded-lg border border-slate-200">
            {numbers.map((number) => {
          ...
          {(globalNumberLimits.length > 0 || commissionNumberLimits.length > 0) && <div className="mt-3 flex flex-wrap gap-3 text-[11px] text-slate-500"><span><i className="mr-1 inline-block h-2 w-2 rounded-sm bg-amber-300" />သတိပေးအဆင့်</span><span><i className="mr-1 inline-block h-2 w-2 rounded-sm bg-red-600" />Limit ကျော်</span></div>}
        </section>
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="font-bold">အနှစ်ချုပ်</h2>
            <span className="text-xs text-slate-500">ယခုအကြိမ်</span>
          </div>
          ...
          <div className="w-full max-w-lg rounded-xl bg-white p-5 shadow-xl">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold">
                အရောင်းစာရင်းပြင်ရန် #{editingSale.receipt_number}
              </h2>
              <button
                type="button"
                onClick={() => setEditingSale(null)}
                className="h-8 w-8 rounded-md border text-slate-500"
              >
                ×
              </button>
            </div>
            <form action={editAction} className="mt-4 space-y-3">
              <input type="hidden" name="sale_id" value={editingSale.id} />
              <label className="block text-sm text-slate-600">
                စာရင်း
                <textarea
                  name="raw_input"
                  defaultValue={editingSale.raw_input}
                  rows={6}
                  className="mt-1.5 w-full rounded-lg border border-slate-300 p-3 text-sm"
                />
              </label>
              <label className="block text-sm text-slate-600">
                ပြင်ဆင်ရတဲ့အကြောင်းပြချက်
                <input
                  name="reason"
                  required
                  placeholder="ဥပမာ - amount မှားနေသောကြောင့်"
              ...
              {editState.success && (
                <p className="text-sm text-emerald-700">{editState.success}</p>
              )}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-red-700">စာရင်းဖျက်ရန်</h2>
              <button
                type="button"
            ...
            <p className="mt-3 text-sm text-slate-600">
              #{deleteTarget.receipt_number} · {deleteTarget.raw_input}
            </p>
            <form action={deleteSale} className="mt-4 space-y-3">
              <input type="hidden" name="sale_id" value={deleteTarget.id} />
              <label className="block text-sm text-slate-600">
                ဖျက်ရတဲ့အကြောင်းပြချက်
                <input
                  name="reason"
                  required
                  placeholder="ဥပမာ - customer မှားပို့ထားသောကြောင့်"
                  className="mt-1.5 h-10 w-full rounded-lg border border-slate-300 px-3 text-sm"
                />
              </label>
              <div className="flex justify-end gap-2">
