...
export default async function Home({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [{ data: aggregatedTotals }, { data: dealerTotals }, { data: globalNumberLimits }, { data: commissionNumberLimits }, { data: allNumberLimit }] = selectedCommission && selectedPeriod ? await Promise.all([
    supabase.rpc("get_number_totals", { p_dealer_id: dealerId, p_commission_id: selectedCommission, p_draw_period_id: selectedPeriod }),
    supabase.rpc("get_dealer_number_totals", { p_dealer_id: dealerId, p_draw_period_id: selectedPeriod }),
    supabase.from("dealer_limits").select("number, max_amount, warning_percent").eq("dealer_id", dealerId).eq("draw_period_id", selectedPeriod).eq("limit_type", "number"),
    supabase.from("commission_number_limits").select("number, max_amount, warning_percent").eq("dealer_id", dealerId).eq("draw_period_id", selectedPeriod).eq("commission_id", selectedCommission),
    supabase.from("dealer_limits").select("max_amount, warning_percent").eq("dealer_id", dealerId).eq("draw_period_id", selectedPeriod).eq("limit_type", "all_number").is("number", null).maybeSingle(),
  ]) : [{ data: [] as { number: string; total_amount: number }[] }, { data: [] as { number: string; total_amount: number }[] }, { data: [] as { number: string; max_amount: number; warning_percent: number }[] }, { data: [] as { number: string; max_amount: number; warning_percent: number }[] }, { data: null as { max_amount: number; warning_percent: number } | null }];
  const totalRows = (aggregatedTotals || []) as { number: string; total_amount: number }[];
  const numberTotals = totalRows.reduce<Record<string, number>>((totals, item) => { totals[item.number] = Number(item.total_amount); return totals; }, {});
  const dealerNumberTotals = ((dealerTotals || []) as { number: string; total_amount: number }[]).reduce<Record<string, number>>((totals, item) => { totals[item.number] = Number(item.total_amount); return totals; }, {});
